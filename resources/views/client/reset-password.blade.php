    ...
    >
      <div class="d-flex justify-content-center"  style="background-color: #1f2122;">
        ...
        <a class="row d-sm-block d-lg-none d-lx-none" style="display: flex; flex-direction: row; align-items: center; justify-content: center;" href="{{ route('home') }}">
              ...
              <div class="col-lg-10 col-md-9 col-xl-6 mx-auto ms-xl-0">
                ...
                <div class="mt-10 mt-lg-5 mb-6 d-lg-block">
                  <span class="d-inline-block d-lg-block h1 mb-4 mb-lg-6 me-3"
                    >🗝</span
                  >
                  <h1 class="ls-tight font-bolder h2">Recuperar a Password!</h1>
                </div>
                <form action="{{ route('password.store') }}" method="POST">
                    @csrf
                  <div class="mb-5">
                    <h6>Preencha o a sua nova password.</h6>
                </div>
                <input type="hidden" name="token" value="{{ $request->route('token') }}">
                <input type="hidden" class="form-control" id="email" name="email" value="{{$request->email}}" />
                <div class="mb-5">
                ...
                <div>
                    <button type="submit" class="btn btn-primary w-full">Submeter</button>
                  </div>
                </form>
              </div>
            </div>
          ...
          >
            <small>Voltar a pagina principal.</small>
            <a
              href="{{ route('home') }}"
